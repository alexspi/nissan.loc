...
class SparesNissController extends SparesApiController
{
...
//        dd($r);
    ...
    public function getNisPicImg($iSID){
        $body = "?key=$iSID&f=cutCanvasKey';";
        $answer = $this->getAnswer($body);
        $r = collect(json_decode($answer));
        return $r;
    }
...
//        dd($market,$model,$modif,$group,$figure,$subfig,$sec,$pnc);
//        $pnc = $this->replaceSS($pnc);
...
//        dd($answer);
    ...
    public function searchNisFrame($vin,$mark,$serial = '',$srcflag = 'y'){
        $body = "t=".static::$_type."&f=".__FUNCTION__.$this->_auth."&vin=$vin&serial=$serial&mark=$mark&srcflag=$srcflag"; //$this->e($body);
       dd($body);
        $answer = $this->getAnswer($body);
        dd($answer);
    ...
    public function searchNISNumber($number,$market,$model='',$modif=''){
        $body = "t=".static::$_type."&f=".__FUNCTION__.$this->_auth."&number=$number&market=$market&model=$model&modif=$modif"; //var_dump($body);exit;
//        dump($body);
        $answer = $this->getAnswer($body);
//        dump($answer);
        $r = collect(json_decode($answer));
